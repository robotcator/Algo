...
void query(int root, int l, int r, int x, int y, int& deg){
  if(x <= l && r <= y){
    deg = ang[root];
  }else{
    update(root);
    int mid = (l+r) >> 1;
    int lc = 2*root, rc = 2*root+1;
    if(x <= mid) query(lc, l, mid, x, y, deg);
    // 因为左子树的节点更改，所以右节点也必须跟着更改
    st[rc].ex = st[lc].ex + (st[rc].ex-st[rc].sx);
    st[rc].ey = st[lc].ey + (st[rc].ey-st[rc].sy);
    st[rc].sx = st[lc].ex;
    st[rc].sy = st[lc].ey;
    if(y > mid) query(rc, mid+1, r, x, y, deg);
    st[root].ex = st[rc].ex;
    st[root].ey = st[rc].ey;
  }
}
...
int main(){
  ...
  while(scanf("%d%d", &n, &c) != EOF){
    ...
    for(int i = 0; i < c; i ++){
      ...
      int deg1, deg2;
      deg1 = deg2 = 0;
      query(1, 1, n, s-1, s, deg1);
      query(1, 1, n, s, s+1, deg2);

      int temp = (deg2-deg1+a+360)%360;
      //cout << temp << endl;
      modify(1, 1, n, s+1, n, temp);
      //for(int i = 1; i <= 7; i ++){
